    ...
    def __init__(self, *args, **kwargs):
        ...
        for y in range(DimTab[1]):
            for x in range(DimTab[0]):
                ...
                self.chk[c] = xbmcgui.ControlImage(
                    ox + cx * x, oy + cy * y, cx, cy, check_image)
                self.chkbutton[c] = xbmcgui.ControlButton(
                    ox + cx * x, oy + cy * y, cx, cy, str(c + 1), font='font1')
                c += 1
        ...
        for c in range(self.DimTabTotal):
            self.chkbutton[c].controlDown(
                self.getbutton(c, "down", DimTab[0], DimTab[1]))
            self.chkbutton[c].controlUp(
                self.getbutton(
                    c, "up", DimTab[0], DimTab[1]))
            self.chkbutton[c].controlLeft(
                self.getbutton(c, "left", DimTab[0], DimTab[1]))
            self.chkbutton[c].controlRight(
                self.getbutton(c, "right", DimTab[0], DimTab[1]))
...
class cInputWindowYesNo(xbmcgui.WindowDialog):
    def __init__(self, *args, **kwargs):
        ...
        self.strActionInfo = xbmcgui.ControlLabel(
            250, 20, 724, 400, kwargs.get('msg'), 'font40', '0xFFFF00FF')
        self.addControl(self.strActionInfo)
        ...
        self.Yesbutton = xbmcgui.ControlButton(
            250 + 520 - 50, 620, 100, 50, 'Yes', alignment=2)
        self.Nobutton = xbmcgui.ControlButton(
            250 + 260 - 70, 620, 140, 50, 'No', alignment=2)
