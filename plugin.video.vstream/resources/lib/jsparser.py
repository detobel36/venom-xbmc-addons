...
# Fonction to return only one parameter from a string with correct closed
# [] () "" and ''
def GetItemAlone(string, separator=' '):
    ...
    while i < l:
        ...
        # Return if the is complete and before the char wanted but not if it's
        # the first one
        ...
        if not c1 and not c2:
            ...
            if ch == '.' and not ((last_char in '0123456789') or (
                    string[i + 1] in '0123456789')):
                n = True
...
class JSBuffer(object):
    ...
    def AddValue(self, value):
        if DEBUG:
            out('ADD (operator: ' + self.__op + ') ' + Ustr(value) +
                ' (' + Ustr(type(value)) + ') a ' + Ustr(self.buf))
    ...
    def Compute(self):
        ...
        # work for all operator
            ...
            if len(self.buf) > 1:
                self.buf[0] = self.opBuf[0] + \
                    str(self.buf[0]) + self.opBuf[1] + str(self.buf[1])
                self.opBuf[0] = ''
                # decale
                del self.opBuf[-1]
                del self.buf[-1]
    ...
    def GetBuffer(self):
        ...
        if self.type == 'Numeric':
            ...
            # There is a bug sometime, long var are signed with L but are int
            # in reality.
...
class JsParserHelper1(object):
    ...
    def process(self, JScode):
        ...
        if not self.t == 'fct':
            ...
            if m and JScode:
                ...
                if self.op == '[':
                    ...
                    if len(
                            self.op) > 1 and self.op[0] == '=' and not self.op[1] == '=':
                        self.op = self.op[0]
                    # a +=-2 situation
        ...
        if self.t == 'fct':
            if DEBUG:
                out('Fonction :' + self.name + ' method: ' +
                    str(self.at1) + ' arg: ' + self.arg)
        elif self.t == 'var':
            if self.property:
                self.at1 = '"' + self.at1 + '"'
            if DEBUG:
        ...
        if self.op:
            if DEBUG:
                out('operation :' + Ustr(self.name) + ' []= ' +
                    Ustr(self.at1) + ' op: ' + str(self.op))
...
class JsParser(object):
    ...
    def ExtractFirstchain(self, string):
        ...
        while l > i:
            ...
            # Dans tout les cas les parenthses doivent etre fermees, ainsi que
            # les crochet
            if not p and not b:
    ...
    def FonctionParser(
        ...
        if DEBUG:
            out('*** fonction > Name: ' + Ustr(name) + ' arg: ' +
                Ustr(arg) + ' function: ' + Ustr(function))
        ...
        if fe:
            if fe == '$':
                ...
                # TODO list copy is rly bad for optimisation, need to make
                # variable list by scope
    ...
    def VarParser(self, vars, allow_recursion, variable, op, JScode):
        ...
        if len(op) == 2:
            ...
            # a+=1 form
                ...
                if isinstance(r, str):
                    self.VarManage(
                        allow_recursion, vars, variable, '"' + r + '"')
                else:
                    self.VarManage(allow_recursion, vars, variable, str(r))
    ...
    def evalJS(self, JScode, vars, allow_recursion):
        ...
        while len(JScode) > 0:
            ...
            if c == "(":
                ...
                if re.search(REG_OP, c2, re.UNICODE):
                    ...
                    self.SetVar(
                        vars, 'TEMPORARY_VARS' + str(allow_recursion), v)
            ...
            if m:
                if not JScode.startswith('String('):
                    ...
                    if v == 'String':
                        self.SetVar(
                            vars, 'TEMPORARY_VARS' + str(allow_recursion), '')
                        JScode = 'TEMPORARY_VARS' + \
                            str(allow_recursion) + JScode
                    # if v == 'Array':
            ...
            if r:
                InterpretedCode.AddValue(
                    self.GetVar(self.HackVars, r.group(1)))
                JScode = JScode[(r.end()):]
                continue
            ...
            if JScode.startswith("function"):
                ...
                if m:
                    ...
                    replac, pos3, v = self.MemFonction(
                        vars, name, m.group(2), openparenthesis, JScode)
                    JScode = replac
                    if v not in replac:
            ...
            while P1.process(JScode):
                ...
                if P1.t == 'var':
                    ...
                    if P1.op:
                        if DEBUG:
                            out('creation/modification/test ' +
                                Var_string + ' ' + P1.op)
                        r, JScode = self.VarParser(
                            vars, allow_recursion, Var_string, P1.op, JScode)

                    else:
                        if not self.IsVar(vars, P1.name):
                            self.PrintVar(vars)
                            raise Exception(
                                'Variable error : ' + Ustr(P1.name))
                    ...
                    r, JScode = self.FonctionParser(
                        vars, allow_recursion, name, fonction, P1.arg, JScode)
                ...
                # normal way
                    ...
                    self.SetVar(
                        vars, 'TEMPORARY_VARS' + str(allow_recursion), r)
                    JScode = 'TEMPORARY_VARS' + str(allow_recursion) + JScode
            ...
            if JScode.startswith('TEMPORARY_VARS' + str(allow_recursion)):
                ...
                JScode = JScode[(
                    len('TEMPORARY_VARS' + str(allow_recursion))):]
                InterpretedCode.AddValue(r)
                continue
            ...
            if c == '?':
                ...
                # A = InterpretedCode.GetPrevious()  Not working if parenthesis
                # absent
            ...
            if JScode[0:2] == '&&' or JScode[0:2] == '||':
                ...
                # for && if the first operand evaluates to false, the second
                ...
                # for || if the result of the first operand is true, the second
                # operand is never evaluated
                if JScode[0:2] == '||':
                    if not A:
                        r = self.evalJS(B, vars, allow_recursion)
        ...
        if DEBUG:
            out(str(allow_recursion) +
                ' : Evalue > ' +
                Ustr(InterpretedCode2) +
                " type " +
                Ustr(type(InterpretedCode2)))
            out('-------------')
    ...
    def GetVar(self, var, variable):
        ...
        if DEBUG:
            out('*** Variable Get > var:' +
                Ustr(variable) + ' index :' + Ustr(index))
        ...
        for j in var:
            if j[0] == variable:
                if index is None:
                    ...
                    # Array
                    # hack, Variable created as list but used as dictionnary,
                    # so need convertion
                    if (isinstance(index, types.StringTypes)) and type(
                            var[var.index(j)][1]) in [list, tuple]:
                        ind = var.index(j)
                        var[ind] = (variable, {})
                        j = (variable, {})
                    ...
                    # dictionnary
                        ...
                        # Variable is fonction so use Object tuple and using
                        # global var for the moment
                        f = self.IsFunc(self.GlobalVar, j[1])
                        if f:
                            f.tuple[index] = value
    ...
    def VarManage(self, allow_recursion, vars, name, value=None):
        ...
        if DEBUG:
            out('*** Variable manager name: ' + Ustr(name) +
                ' value: ' + Ustr(value) + ' ' + str(type(value)))
        ...
        if DEBUG:
            if index is None:
                out('*** Variable in parser => ' +
                    Ustr(name) + ' = ' + Ustr(value))
            else:
                out('*** Variable in parser => ' + Ustr(name) +
                    '[' + Ustr(index) + ']' + ' = ' + Ustr(value))
    ...
    def Parse(self, JScode, vars, allow_recursion=MAX_RECURSION):
        ...
        # So we extract all functions first, and replace them by a simple call
        # in the code, if they are self invoked
        ...
        if 'function' in JScode:
            ...
            while True:
                ...
                if m:
                    ...
                    replac, pos3, xyz = self.MemFonction(
                        vars, name, m.group(3), openparenthesis, chain)
        ...
        while True:
            ...
            if m:
                ...
                if name == 'for':
                    ...
                    # loop
                    while self.CheckTrueFalse(
                            self.evalJS(t, vars, allow_recursion)):
                        # fonction
                ...
                if name == 'do':
                    ...
                    # and now the loop
                    while self.CheckTrueFalse(
                            self.evalJS(e, vars, allow_recursion)):
                        # fonction
                        self.Parse(f, vars, allow_recursion)
                        if self.Break:
                            self.Break = False
                ...
                if name == 'with':
                    ...
                    out('> With fonction : exp=' + arg +
                        ' values=' + str(member_list))
                    # print('Before: ' + f)
                    ...
                    if type(member_list) in [type]:
                        for i in member_list.__dict__:
                            f = re.sub(
                                r'[^\w]' + str(i) + '[^\\w]', sub, f, re.DOTALL)
                    else:
                        for i in member_list:
                            f = re.sub(
                                r'[^\w]' + i + '[^\\w]', sub, f, re.DOTALL)
...
class Basic(object):
    ...
    def eval(self, arg):
        if DEBUG:
            out('To eval >' + arg)
        r = self._JSParser.Parse(
            RemoveGuil(arg),
            self._JSParser.FastEval_vars,
            self._JSParser.FastEval_recur)
        return r
