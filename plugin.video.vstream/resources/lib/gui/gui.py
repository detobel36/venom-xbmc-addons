...
class Gui:
    ...
    def addNewDir(
        ...
        if thumbnail == '':
            ...
            oGuiElement.setPoster(thumbnail)
        ...
        title = output_parameter_handler.getValue('sMovieTitle')
        if title:
            oGuiElement.setFileName(title)
        else:
            oGuiElement.setFileName(label)
    ...
    def addEpisode(
        ...
        if saisonUrl:   # Retenu depuis "addSeason"
            ...
            output_parameter_handler.addParameter(
                'saisonUrl', input_parameter_handler.getValue('siteUrl'))
            output_parameter_handler.addParameter(
    ...
    def addNext(self, sId, function, label, output_parameter_handler):
        oGuiElement = GuiElement()
        oGuiElement.setSiteName(sId)
        oGuiElement.setFunction(function)
        oGuiElement.setTitle('[COLOR teal]' + label + ' >>>[/COLOR]')
    ...
    def addText(self, sId, label='', sIcon='none.png'):
    ...
    # affiche tag HD
        ...
        output_parameter_handler.addParameter(
            'sTitleWatched', oGuiElement.getTitleWatched())
    ...
    def _createListItem(self, oGuiElement, oListItem=None):
        ...
        data = {
            key: val for key,
            val in oGuiElement.getItemValues().items() if val != ""}
    ...
    def createContexMenuPageSelect(
        ...
        output_parameter_handler.addParameter(
            'OldFunction', oGuiElement.getFunction())
    ...
    def createContexMenuBookmark(
            ...
            output_parameter_handler=''):
        output_parameter_handler.addParameter(
            'sCleanTitle', oGuiElement.getCleanTitle())
        output_parameter_handler.addParameter('sId', oGuiElement.getSiteName())
        output_parameter_handler.addParameter(
            'sFav', oGuiElement.getFunction())
    ...
    def createContexMenuba(self, oGuiElement, output_parameter_handler=''):
        output_parameter_handler = OutputParameterHandler()
        output_parameter_handler.addParameter('title', oGuiElement.getTitle())
        output_parameter_handler.addParameter(
            'sFileName', oGuiElement.getFileName())
        output_parameter_handler.addParameter('sYear', oGuiElement.getYear())
        output_parameter_handler.addParameter(
            'trailerUrl', oGuiElement.getTrailer())
        output_parameter_handler.addParameter('sMeta', oGuiElement.getMeta())
    ...
    def __createItemUrl(self, oGuiElement, output_parameter_handler=''):
        ...
        # Pour gérer l'enchainement des épisodes
        output_parameter_handler.addParameter(
            'sSeason', oGuiElement.getSeason())
        output_parameter_handler.addParameter(
            'sEpisode', oGuiElement.getEpisode())
        ...
        if len(oGuiElement.getFunction()) == 0:
            sItemUrl = '%s?site=%s&title=%s&%s' % (sPluginPath, oGuiElement.getSiteName(
    ...
    def viewInfo(self):
        if addon().getSetting('information-view') == "false":
            ...
            input_parameter_handler = InputParameterHandler()
            sCleanTitle = input_parameter_handler.getValue('title') if input_parameter_handler.exist(
    ...
    def selectPage(self):
        ...
        if newNum:
            try:
                ...
                sParams = output_parameter_handler.getParameterAsUri()
                sTest = '%s?site=%s&function=%s&%s' % (
                    sPluginPath, sId, function, sParams)
                xbmc.executebuiltin('Container.Update(%s)' % sTest)
            except BaseException:
                return False
    ...
    def setWatched(self):
        if True:
            ...
            title = input_parameter_handler.getValue('sTitleWatched')
            sCat = input_parameter_handler.getValue('sCat')
            if not title:
                return
    ...
    def showNofication(self, title, iSeconds=0):
        return False

    def showError(self, title, sDescription, iSeconds=0):
        return False

    def showInfo(self, title, sDescription, iSeconds=0):
        return False
    ...
    def addSearchResult(self, oGuiElement, output_parameter_handler):
        ...
        Gui.searchResults[searchSiteId].append(
            {'guiElement': oGuiElement, 'params': copy.deepcopy(output_parameter_handler)})
        Gui.searchResultsSemaphore.release()
