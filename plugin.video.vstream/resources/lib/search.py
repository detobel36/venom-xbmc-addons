...
class Search:
    ...
    def searchGlobal(self, search_text='', cat=''):
        try:
            ...
            for plugin in listPlugins:
                ...
                if pluginId in searchResults.keys() and (
                        len(searchResults[pluginId]) > 0):  # Au moins un résultat
                    count += 1
    ...
    def _progressUpdate(self, numberResult):
        if not self.autoPlayVideo and not self._progressIsCancel():
            ...
            message += "\n"
            message += (self.addons.VSlang(31208) %
                        (", ".join(self.listRemainingPlugins[0:7])))
            if len(self.listRemainingPlugins) > 7:
                message += ", ..."
            self.progress_.VSupdate(
                self.progress_, self.progressTotal, message, True)
    ...
    def quickSearch(self, autoPlayVideo=False):
        try:
            ...
            listPlugins = self._initSearch(
                searchInfo['title'], searchInfo['cat'])
            ...
            listThread = self._launchSearch(
                listPlugins, self._quickSearchForPlugin, [searchInfo])

            if autoPlayVideo:
                while len(
                        self.listRemainingPlugins) > 0 and self._continueToSearch():
                    self.eventFindOneLink.wait()
                    self.eventFindOneLink.clear()
                    self._tryToAutoPlaySpecificCriteria(
                        Search.MAX_NUMBER_CRITERIA)
    ...
    def _progressInit(self):
        if not self.autoPlayVideo:
            # kodi 17 vire la fenetre busy qui se pose au dessus de la barre de
            # Progress
            try:
    ...
    def _continueToSearch(self):
        return not (self.findAndPlay or self._monitorAbortRequest()
    ...
    def _initSearch(self, searchText, searchCat):
        try:
            ...
            self.listRemainingPlugins = [plugin['name']
                                         for plugin in listPlugins]
    ...
    def _tryToAutoPlaySpecificCriteria(self, numberCriteria):
        if numberCriteria in self.allVideoLink:
            for searchResult in self.allVideoLink[numberCriteria]:
                # Si on ne doit pas arrêter de chercher et si on a pas encore
                # tester ce résultat
                if self._continueToSearch() and searchResult['params'].getValue(
                        'playTest') == 'false':
                    ...
                    find = self._playHosterGui(
                        searchResult['guiElement'].getFunction(), [
    ...
    def _deepSearchLoop(self, searchResult, searchInfo):
        if self._continueToSearch():
            numberOfCriteria = self._getScoreOfThisResult(
            ...
            if numberOfCriteria > 0:
                ...
                if siteId == 'HosterGui':
                    ...
                    videoParams.addParameter(
                        'searchSiteId', siteId)  # On concerve le site d'origine
                    Gui().emptySearchResult(siteId)
                    self._executePluginForSearch(
                        siteId, functionName, videoParams)

                    pluginResult = Gui().getSearchResult()
                    for newSearchResult in pluginResult[siteId]:
                        if self._continueToSearch():
                            # On garde toutes les données qu'on avait au par
                            # avant
                            newSearchResult['params'].mergeUnexistingInfos(
                                videoParams)
                            # On concerve le site d'origine
                            newSearchResult['params'].addParameter(
                                'searchSiteId', siteId)

                            if newSearchResult['guiElement'].getSiteName() == siteId and \
                                    newSearchResult['guiElement'].getFunction() == functionName:
                                VSlog(
                                    "Will not loop on the same function: " +
                                    str(siteId) +
                                    "." +
                                    str(functionName))
                            else:
                                self._deepSearchLoop(
                                    newSearchResult, searchInfo)
                        else:
                            break
    ...
    def _quickSearchForPlugin(self, plugin, searchInfo):
        if self._continueToSearch():
            ...
            if pluginId in searchResults and len(
                    searchResults[pluginId]) > 0:  # Au moins un résultat
                pluginResult = searchResults[pluginId][:]

                for searchResult in pluginResult:
                    searchResult['params'].addParameter(
                        'searchSiteName', pluginName)
    ...
    def _getSearchInfo(self):
        ...
        if input_parameter_handler.exist('cat'):
            searchCat = int(input_parameter_handler.getValue('cat'))
        else:
            searchCat = -1

        if input_parameter_handler.exist('title'):
            searchTitle = self._removeNonLetterCaracter(
                input_parameter_handler.getValue('title'))
        else:
            searchTitle = ""

        if input_parameter_handler.exist('year'):
            searchYear = str(input_parameter_handler.getValue('year'))
        else:
            searchYear = ""
    ...
    def _isMovieTitleCorrect(self, result, searchInfo):
        ...
        if resultTitle:
            if self._checkAllSearchWordInTitle(
                ...
                if searchInfo['title'].lower() == self._removeNonLetterCaracter(
                        resultTitle).lower():
                    return 2
                else:
                    return 1
        ...
        try:
            ...
            if updateProcess:
                ...
                self._progressUpdate(
                    sum(map(len, Gui().getSearchResult().values())))
    ...
    def _playHosterGui(self, function, parameters=None):
        VSlog("Execute HosterGui." + str(function))
        plugins = __import__(
            'resources.lib.gui.hoster',
            fromlist=['HosterGui']).HosterGui()
        function = getattr(plugins, function)
        if parameters:
