...
class TMDb:
    ...
    # ancien : 'plugin://plugin.video.youtube/?action=play_video&videoid=%s'
                    ...
                    if not movie:
                        for searchMovie in meta['results']:
                            if searchMovie['genre_ids'] and 99 not in searchMovie['genre_ids']:
                                ...
                                # controle supplémentaire sur l'année meme si
                                # déjà dans la requete
                                if year:
    ...
    def _format(self, meta, name, media_type=""):
        ...
        if 'trailer' in meta and meta['trailer']:   # Lecture du cache
            ...
            try:    # Recherche de la BA en français
                ...
                pass
        # Trailer d'une série retourné par TMDB
                ...
                if len(trailers['results']) > 0:
                    for trailer in trailers['results']:
                        if trailer['type'] == 'Trailer' and trailer['site'] == 'YouTube':
                            # Au moins c'est un trailer, pas forcement français
        ...
        if 'credits' in meta and meta['credits']:
            # Code from
            # https://github.com/jurialmunkey/plugin.video.themoviedb.helper/blob/matrix/resources/lib/tmdb/mapping.py
            cast_list = []
            ...
            for i in sorted(cast_list, key=lambda k: k.get('order', 0)):
                ...
                if not cast_item:
                    ...
                    if i.get('profile_path'):
                        cast_item['thumbnail'] = self.poster + \
                            i['profile_path']
            if cast_item:
                cast.append(cast_item)
    ...
    def _cache_save_movie(self, meta, name, year):
        # year n'est pas forcement l'année du film mais l'année utilisée pour
        # la recherche
    ...
    def get_meta(
        ...
        if not update:
            # Obligatoire pour pointer vers les bonnes infos dans la base de
            # données
            if not tmdb_id:
