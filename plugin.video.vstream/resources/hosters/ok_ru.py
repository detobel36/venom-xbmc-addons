...
class cHoster(iHoster):
    ...
    def getHostAndIdFromUrl(self, url):
        pattern = 'https*:\\/\\/.*?((?:(?:ok)|(?:odnoklassniki))\\.ru)\\/.+?\\/([0-9]+)'
        parser = Parser()
    ...
    def _getMediaLinkForGuest(self, auto_play=False):
        ...
        s_id = v[1]
        host = v[0]
        web_url = 'http://' + host + '/videoembed/' + s_id
        ...
        St = requests.Session()
        html_content = St.get(web_url).content.decode('utf-8')
        parser = Parser()
        ...
        html_content = parser.abParse(
            ...
            14)
        html_content = cUtil().removeHtmlTags(html_content)
        html_content = cUtil().unescape(html_content)
