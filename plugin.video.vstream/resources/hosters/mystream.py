    ...
    def _getMediaLinkForGuest(self, auto_play=False):
        ...
        if api_call:
            return True, api_call + '|User-Agent=' + UA + '&Referer=' + \
                self._url + '&Origin=https://embed.mystream.to'
...
def temp_decode(data):
    ...
    if pos:
        first_group = first_group.replace(
            '$.__+',
            't').replace(
            '$._+',
            'u').replace(
        ...
        if js:
            ...
            first_group = re.sub(
                '(\\(\\!\\[\\]\\)\\[.+?\\]+)', 'l', first_group)
        try:
            final_data = first_group.encode('ascii').decode(
                'unicode-escape').encode('ascii') .decode('unicode-escape')
            return final_data
