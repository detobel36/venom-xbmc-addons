...
class cHoster(iHoster):
    ...
    def _getMediaLinkForGuest(self):
        ...
        if 'error' in dictData.keys():
            dialog().VSinfo(
                dictData['error'].upper().replace(
                    '_', ' '), self.getPluginIdentifier())
            # si realdebrid ne prend pas en charge ce type de lien, on retourne
            # le lien pour utiliser un autre hoster
        ...
        if api_call:
            try:
                ...
                VSlog('Hoster RealDebrid - play : %s/ ... /%s' %
