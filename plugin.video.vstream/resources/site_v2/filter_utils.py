...
class FilterUtils:
    """
    Méthodes utilisées pour filtrer les résultats
    """
    ...
    def is_not_ignore_by_filters(item_filter: Dict[str, str], value_used_to_filter) -> bool:
        ...
        """
        if 'eq' in item_filter and value_used_to_filter != item_filter['eq']:
            return False

        if 'neq' in item_filter and value_used_to_filter == item_filter['neq']:
            return False

        if 'contains' in item_filter and item_filter['contains'] not in value_used_to_filter:
            return False

        if 'not_contains' in item_filter and item_filter['not_contains'] in value_used_to_filter:
            return False
