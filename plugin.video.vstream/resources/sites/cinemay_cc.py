...
def load():
    ...
    oGui.addDir(SITE_IDENTIFIER, MOVIE_LIST[1], 'Films (Liste alphabétique)', 'az.png', output_parameter_handler)
    ...
    oGui.addDir(SITE_IDENTIFIER, SERIE_LIST[1], 'Séries (Liste alphabétique)', 'az.png', output_parameter_handler)
...
def showAlpha(sTypeSerie=''):
    ...
    for sTitle, sUrl in liste:
        output_parameter_handler.addParameter('siteUrl', URL_MAIN + 'letter/' + sUrl + str(sType) + sTypeSerie)
        oGui.addDir(
            SITE_IDENTIFIER,
            'showMovies',
            'Lettre [COLOR coral]' +
...
def showMovieYears(sTypeSerie=''):
    ...
    for i in reversed(range(2001, 2023)):  # pas grand chose 32 - 90
        Year = str(i)
        output_parameter_handler.addParameter('siteUrl', URL_MAIN + 'annee/' + Year + sTypeSerie)
...
def showSelectType():
    ...
    if 'class="num-epi">' in sHtmlContent:
        ...
        oGui.addTV(SITE_IDENTIFIER, 'showSaison', sMovieTitle, '', sThumb, sDesc, output_parameter_handler)
    else:
